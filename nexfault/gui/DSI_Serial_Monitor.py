    ...
    try:
        ...
        if simulated:
            ...
            if ser_dsi is None or not ser_dsi.is_open:
                dpg.add_text(
                    f"[Capture] Error: DSI Port not open\n\n", parent="log_window_dsi"
                )
                return
        ...
        if rows:
            ...
            dpg.add_text(
                f"[Capture] Saved JSON: {json_path}\n\n", parent="log_window_dsi"
            )
    except Exception as e:
        dpg.add_text(f"[Capture Error] {e}\n\n", parent="log_window_dsi")
    finally:
        sim_again = (
            str(dpg.get_value("dsi_port")).strip().lower().startswith("simulated")
        )
...
def send_command_callback():
    ...
    try:
        ...
        transport_val = 0
        if dpg.get_value("use_uart"):
            transport_val |= 1  # TRANSPORT_UART
        if dpg.get_value("use_modbus"):
            transport_val |= 2  # TRANSPORT_MODBUS

        if transport_val == 0:
            dpg.add_text(
                "[Error] Please select at least one Transport type\n\n",
                parent="log_window_dsi",
            )
            return
        ...
        if main_command == "Ping":
            ...
            if inj_type == "Byte Drop":
                ...
                dpg.add_text(
                    f"[TX] Inject BitFlip (every_n={start_offset}, bits={length}, mode={mode}, pattern='{pattern_str}')\n\n",
                    parent="log_window_dsi",
                )
                ...
                dpg.add_text(
                    f"[TX] Inject Phantom ({mode}, offset={start_offset}, payload='{phantom_payload}')\n\n",
                    parent="log_window_dsi",
                )
...
with dpg.window(label="DSI Monitor", width=1900, height=980, pos=(0, 0)):
    ...
    with dpg.group(horizontal=True):
        ...
        dpg.add_button(
            label="Connect DSI", tag="btn_connect_dsi", callback=toggle_dsi_connection
        )
    ...
    with dpg.group(horizontal=True):
        ...
        dpg.add_progress_bar(
            tag="progress_bar", default_value=0.0, width=200, overlay="Progress"
        )

    with dpg.group(tag="injection_params_group", show=False):
        with dpg.group(horizontal=True):
            dpg.add_combo(
                items=["Byte Drop", "Bit Flip", "Phantom Byte"],
